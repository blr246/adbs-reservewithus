...
# encoding: utf-8
"""
Reservewithus - data generation
This script generates setup data for the ReserveWithUs case study
The specification files and generated data are placed in the local directory
See main comments (at the bottom of this file) for details on generated data.
...
def gen(numrows, targfile, specfile, numkeys):
    # Extract columns specifications from specfile
    print "reading specfile for "+targfile+"..."
    f = open(specfile)
...
def main(argv=None):
    print "This will take about an hour (with 2 GB of RAM)..."
    # 10^6 customers
    gen(1000000, 'customer.data', 'customerspec', 1)
    # 5*10^4 hotels
    gen(50000, 'hotel.data', 'hotelspec', 1)

    # 6 room_types per hotel (on average)
    # 3*10^5 room_desc tuples
    gen(300000, 'roomtype.data', 'roomtypespec',1)

    # 1 months ahead of room/dates deals - 3 room types per hotel over 1 month (30 days)
    # ~ 4.5*10^6 room_dates = 30 * 15*10^4
    # 2 steps generation for meaningful date_start, date_stop intervals
    gen(4500000, 'roomdate.data', 'roomdatespec',2)
